...
<!-- user script-->
<script
	src="${pageContext.request.contextPath}/resources/js/boardProduct/updateReviewForm.js"></script>
...
<body>
	<div class="flex-grow-1 m-5 p-5">
		<div class="d-flex row">
		 ...
		 <form id="writeReviewForm" method="post" action="updateReview" enctype="multipart/form-data">
		 	<!-- 상품후기 -->
	 		...
	 		<input id="pid" type="hidden" name="pid" class="form-control"
	 		 style="border:none; border-bottom: 1px solid #ddd;" value="${boardProduct.pid}">
	 		 <input id="bpnumber" type="hidden" name="bpnumber" class="form-control"
	 		 style="border:none; border-bottom: 1px solid #ddd;" value="${boardProduct.bpnumber}">	 
		 	<div class="input-group mt-4">
		 		...
		 		<input id="bptitle" type="text" name="bptitle" class="form-control" placeholder="제목을 입력해주세요. (최대 125자)"
		 		 style="border:none; border-bottom: 1px solid #ddd;" value="${boardProduct.bptitle}">
		 	</div>
		 			 	
		 	<div class="input-group mt-4">
		 		<textarea id="bpcontent" name="bpcontent" class="form-control"
		 			style="width: 100%; height: 300px;">${boardProduct.bpcontent}</textarea>
		 	</div>
		 	...
		 	<div class="input-group mt-4 row">
		 		...
		 		<div class="col-8" id="bpattachPreview" style="width:120px;height:120px;">
		 			<div class="fileBox" style="position:relative;">
						<img src="boardProductImageDownload?bpnumber=${boardProduct.bpnumber}" class="mt-0" width="100px" height="100px">
						<button data-index="${i}" onclick="deleteSelectImage(this, 'body')" class="btn btn-light btn-sm" type="button" style="position:absolute;top:0px;right:-5px;\"><i class="bi bi-x"></i></button>
					</div>
		 		</div>
		 	</div>
		 	
		 	<div class="mt-3 d-flex">
		 		<button type="submit" class="btn btn-dark me-2" style="margin-left: auto;">수정</button>
		 		<a class="btn btn-outline-secondary" href="${pageContext.request.contextPath}/product/product_detail?pid=${boardProduct.pid}">목록</a>
		 	</div>
		 </form>
		</div>
	</div>
